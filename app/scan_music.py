...
def read_metadata(path: Path):
    artist, title, album, genre, year, duration = "", path.stem, "", "", None, None
    try:
...
def main():
    ...
    if not music_dir.is_dir():
        raise SystemExit(f"Muziekbron ontbreekt of is geen map: {music_dir}")
    ...
    for path in music_dir.rglob("*"):
        ...
        conn.execute("""
            ...
            ON CONFLICT(path) DO UPDATE SET artist=excluded.artist, title=excluded.title,
                album=excluded.album, genre=excluded.genre, year=excluded.year,
                duration=excluded.duration, mtime=excluded.mtime, enabled=1,
