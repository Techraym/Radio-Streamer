...
echo "=== NRG Radio 1.1 installatiecontrole ==="

if [ "$(id -u)" -eq 0 ]; then ok "root/sudo"; else bad "voer uit met sudo"; fi

if [ -r /etc/os-release ]; then
  . /etc/os-release
  [ "${ID:-}" = "debian" ] && ok "${PRETTY_NAME:-Debian}" || bad "Debian vereist; gevonden: ${PRETTY_NAME:-onbekend}"
else
  bad "/etc/os-release ontbreekt"
fi

case "$(dpkg --print-architecture 2>/dev/null || uname -m)" in
  amd64|arm64|x86_64|aarch64) ok "architectuur ondersteund" ;;
  *) note "architectuur niet standaard getest" ;;
esac
...
if mountpoint -q "$MUSIC_MOUNT" 2>/dev/null; then
  ...
  case ",$opts," in
    *,ro,*) ok "$MUSIC_MOUNT is read-only" ;;
    *) bad "$MUSIC_MOUNT is NIET read-only: $opts" ;;
  esac
else
  bad "$MUSIC_MOUNT is niet gemount"
fi

if [ -w "$MUSIC_MOUNT" ] 2>/dev/null; then
  bad "streamer kan schrijven naar $MUSIC_MOUNT"
else
  ok "streamer heeft geen schrijfrechten op muziekopslag"
fi
